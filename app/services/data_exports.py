...
class DataExportService:
    async def create_export(self, user: User, data: DataExportCreateRequest) -> DataExportResponse:
        if not await self._has_any_data(user.id, data.start_date, data.end_date, data.data_types):
            raise HTTPException(
                status_code=status.HTTP_422_UNPROCESSABLE_CONTENT, detail="선택한 기간에 데이터가 없습니다."
            )
    ...
    async def build_download(self, user: User, export_id: str) -> tuple[str, str, bytes]:
        ...
        if export is None:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="요청한 내보내기 파일을 찾을 수 없습니다."
            )
    ...
    async def _collect_rows(user_id: int, start: date, end: date, data_types: list[str]) -> list[dict]:
        ...
        if DataExportType.BLOOD_PRESSURE.value in data_types:
            records = await VitalRecord.filter(
                user_id=user_id, record_date__gte=start, record_date__lte=end, measure_type__startswith="BP_"
            )
            rows.extend(
                {
                    "type": "BLOOD_PRESSURE",
        ...
        if DataExportType.BLOOD_GLUCOSE.value in data_types:
            records = await VitalRecord.filter(
                user_id=user_id, record_date__gte=start, record_date__lte=end, measure_type__startswith="GLUCOSE_"
            )
            rows.extend(
                {
                    "type": "BLOOD_GLUCOSE",
            ...
            rows.extend(
                {
                    ...
                    "calories": r.calories,
                    "sodium_mg": float(r.sodium_mg) if r.sodium_mg is not None else None,
                    "memo": r.memo,
                }
                for r in records
            )
            ...
            rows.extend(
                {
                    "type": "RENAL",
                    "date": str(r.record_date),
                    "creatinine": float(r.creatinine) if r.creatinine is not None else None,
                    "egfr": float(r.egfr) if r.egfr is not None else None,
                    "bun": float(r.bun) if r.bun is not None else None,
                    "urine_protein_pos": r.urine_protein_pos,
                }
                for r in records
            )
            ...
            rows.extend(
                {
                    ...
                    "diet_score": float(r.diet_score) if r.diet_score is not None else None,
                    "memo": r.memo,
                }
                for r in records
            )
    ...
    def _render(export_format: str, rows: list[dict]) -> tuple[bytes, str, str]:
        ...
        if export_format == DataExportFormat.XLSX.value:
            return (
                DataExportService._csv_bytes(rows),
